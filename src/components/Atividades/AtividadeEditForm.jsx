// src/components/Atividades/AtividadeEditForm.jsx
...
/**
 * Função para formatar a data para o BigQuery
 * Converte 'YYYY-MM-DDTHH:MM' para 'YYYY-MM-DD HH:MM:SS'
 */
const formatarDataParaBigQuery = (dateTime) => {
  ...
  // Converte 'YYYY-MM-DDTHH:MM' para 'YYYY-MM-DD HH:MM:00'
  return dateTime.replace('T', ' ') + ':00';
};

const AtividadeEditForm = ({ clienteId, atividade = null, onClose, onSubmit }) => {
  ...
  useEffect(() => {
    const fetchPadraoAtividades = async () => {
      try {
        ...
        const response = await axios.get( `https://api.multiluzsolar.com.br/app1000/api/padraoatividades` || `http://localhost:${port}/api/padraoatividades`, {
          headers: {
    ...
    try {
      ...
      if (atividade) {
        ...
        response = await axios.put(
          `https://api.multiluzsolar.com.br/app1000/api/atividades/${atividade.IdAtividade}` ||  `http://localhost:${port}/api/atividades/${atividade.IdAtividade}`,
          dataToSend,
          {
            headers: {
              Authorization: `Bearer ${token}`,
        ...
        response = await axios.post(
          `https://api.multiluzsolar.com.br/app1000/api/atividades` || `http://localhost:${port}/api/atividades`,
          dataToSend,
          {
            headers: {
              Authorization: `Bearer ${token}`,
...
export default AtividadeEditForm;
